...
static inline int hanr_client_create_packet(struct rte_mempool* mp,
    ...
    uint32_t srcaddr = IPV4_ADDR(192, 168, 250, 2);
    uint32_t dstaddr = IPV4_ADDR(192, 168, 250, 1);
    uint16_t srcport = (uint16_t)rand_r(&seed);
    uint16_t dstport = 9999;
    uint16_t ether_len, ip_len, udp_len;
    uint16_t pktlen;

    static uint8_t srcmac[] = {0xA0, 0x36, 0x9F, 0x84, 0x00, 0xEC};
    static uint8_t dstmac[] = {0xA0, 0x36, 0x9F, 0x84, 0x02, 0x5C};
...
int hanr_tx_main(void* arg) {
    ...
    while (!hanr_quit) {
        ...
        ret = hanr_client_process_tx(&hanr_conf, mbuf, burst);
