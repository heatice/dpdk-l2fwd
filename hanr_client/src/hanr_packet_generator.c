...
static void copy_buf_to_pkt_segs(void* buf,
                                 unsigned len,
                                 struct rte_mbuf* pkt,
                                 unsigned offset) {
    struct rte_mbuf* seg;
...
static inline void copy_buf_to_pkt(void* buf,
                                   unsigned len,
                                   struct rte_mbuf* pkt,
                                   unsigned offset) {
...
void initialize_eth_header(struct rte_ether_hdr* eth_hdr,
    ...
    if (vlan_enabled) {
        struct rte_vlan_hdr* vhdr =
            (struct rte_vlan_hdr*)((uint8_t*)eth_hdr +
...
void initialize_arp_header(struct rte_arp_hdr* arp_hdr,
                           struct rte_ether_addr* src_mac,
                           struct rte_ether_addr* dst_mac,
                           uint32_t src_ip,
                           uint32_t dst_ip,
...
uint16_t initialize_udp_header(struct rte_udp_hdr* udp_hdr,
                               uint16_t src_port,
                               uint16_t dst_port,
...
uint16_t initialize_tcp_header(struct rte_tcp_hdr* tcp_hdr,
                               uint16_t src_port,
                               uint16_t dst_port,
                               uint16_t pkt_data_len) {
    uint16_t pkt_len;
...
uint16_t initialize_sctp_header(struct rte_sctp_hdr* sctp_hdr,
                                uint16_t src_port,
                                uint16_t dst_port,
                                uint16_t pkt_data_len) {
    uint16_t pkt_len;
...
uint16_t initialize_ipv6_header(struct rte_ipv6_hdr* ip_hdr,
                                uint8_t* src_addr,
                                uint8_t* dst_addr,
                                uint16_t pkt_data_len) {
    ip_hdr->vtc_flow = 0;
...
uint16_t initialize_ipv4_header(struct rte_ipv4_hdr* ip_hdr,
                                uint32_t src_addr,
                                uint32_t dst_addr,
                                uint16_t pkt_data_len) {
    uint16_t pkt_len;
...
uint16_t initialize_ipv4_header_proto(struct rte_ipv4_hdr* ip_hdr,
                                      uint32_t src_addr,
                                      uint32_t dst_addr,
                                      uint16_t pkt_data_len,
                                      uint8_t proto) {
...
static uint64_t alloc_failed[RTE_MAX_LCORE];
int generate_packet_burst(struct rte_mempool* mp,
    ...
    for (nb_pkt = 0; nb_pkt < nb_pkt_per_burst; nb_pkt++) {
        ...
        if (pkt == NULL) {
            alloc_failed[rte_lcore_id()]++;
            printf("alloc pktbuf failed: lcore: %d, fails: %lu\n",
                   rte_lcore_id(), alloc_failed[rte_lcore_id()]);
            printf("mempool: size: %u, populated: %u\n",
                   rte_mempool_avail_count(mp), rte_mempool_in_use_count(mp));
            rte_exit(EXIT_FAILURE, "Alloc Failed.\n");
        nomore_mbuf:
            if (nb_pkt == 0)
                return -1;
            break;
        ...
        if (payload != NULL) {
            copy_buf_to_pkt(payload, plen, pkt, payload_offset);
        }
...
int generate_packet_burst_proto(struct rte_mempool* mp,
    ...
    for (nb_pkt = 0; nb_pkt < nb_pkt_per_burst; nb_pkt++) {
        ...
        if (ipv4) {
            ...
            switch (proto) {
                case IPPROTO_UDP:
                    copy_buf_to_pkt(proto_hdr, sizeof(struct rte_udp_hdr), pkt,
                                    eth_hdr_size + sizeof(struct rte_ipv4_hdr));
                    break;
                case IPPROTO_TCP:
                    copy_buf_to_pkt(proto_hdr, sizeof(struct rte_tcp_hdr), pkt,
                                    eth_hdr_size + sizeof(struct rte_ipv4_hdr));
                    break;
                case IPPROTO_SCTP:
                    copy_buf_to_pkt(proto_hdr, sizeof(struct rte_sctp_hdr), pkt,
                                    eth_hdr_size + sizeof(struct rte_ipv4_hdr));
                    break;
                default:
                    break;
...
int
hanr_generate_packet_burst(struct rte_mempool *mp, struct rte_mbuf **pkts_burst,
		int idx,
		...
		int nb_pkt_per_burst, uint16_t pkt_len, uint8_t nb_pkt_segs, uint64_t ts)
{
	int i, nb_pkt = 0;
	size_t eth_hdr_size;
	size_t payload_offset = 0;
...
nomore_mbuf:
		...
		if (ipv4) {
			...
			copy_buf_to_pkt(udp_hdr, sizeof(*udp_hdr), pkt,
				eth_hdr_size + sizeof(struct rte_ipv4_hdr));
			payload_offset += sizeof(struct rte_ipv4_hdr) + sizeof(struct rte_udp_hdr);
		} else {
			copy_buf_to_pkt(ip_hdr, sizeof(struct rte_ipv6_hdr),
				pkt, eth_hdr_size);
			copy_buf_to_pkt(udp_hdr, sizeof(*udp_hdr), pkt,
				eth_hdr_size + sizeof(struct rte_ipv6_hdr));
			payload_offset += sizeof(struct rte_ipv6_hdr) + sizeof(struct rte_udp_hdr);
		}
