...
int hanr_t2_do_query(struct hanr_client_conf *cfg,struct hanr_packet *pkt, struct hanr_msg_query_request *query_request){
    int ret = 0;
    printf("t2 process query packets\n");
    ...
    if(entry!=NULL){
        ...
        if (ret <= 0) {
            printf("T2 reply TX failed.\n");
            return 0;
        }

    }else{
        printf("not find in the cache,now find in the db.\n");
        ...
        if(query_request->prefix == PREFIX){
            ...
            {
                printf("can't lookup data in cache and db.\n");
        ...
        {
            printf("not match this prefix field.\n");
