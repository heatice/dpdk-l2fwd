...
static inline int hanr_client_create_query_request(struct rte_mempool* mp,
    ...
    struct hanr_msg_query_reply msg = {0};
    msg.type = HANR_MSG_QUERY_REPLY;
    //msg.prefix = PREFIX;
    msg.len = htons(sizeof(struct hanr_msg_query_reply));
    msg.request_id = htonl(rid);
    msg.timestamp = htonl(time(NULL));
    msg.status = 1;
    rte_memcpy(msg.na,tmp_na,HANR_NA_LEN);
    snprintf(msg.eid, HANR_EID_LEN, "eid-%010d", rid);
